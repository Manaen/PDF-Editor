...
//            listView.getItems().add(file.getName());
    ...
    private void mergePDFmain(List<String> files) throws IOException{
        ...
        pdfMerger.setDestinationFileName(files.get(0).substring(0, files.get(0).length() - 4)+"j_new.pdf");
        try {
            for(String s: files)
    ...
    private void putOnPagination(Pagination paging, File file){
        ...
        paging.setPageFactory((Integer index) -> {
            model = new MainPdf(Paths.get(file.getAbsolutePath()));
    ...
    void handleSpritterButtonAction(ActionEvent event) {
        ...
        try {
            ...
            while(iterator.hasNext()) {
               ...
               pd.save(srcFile.substring(0, srcFile.length() - 4) + "_p_" + i++ + "_new.pdf");
            }
            document.close();
